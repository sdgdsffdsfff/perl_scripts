...
def run_timedrift_with_reboot(test, params, env):
    ...
    try:
        ...
        (ht0, gt0) = xen_test_utils.get_time(session, time_command,
                                             time_filter_re, time_format)

        # Reboot
        for i in range(reboot_iterations):
            # Get time before current iteration
            (ht0_, gt0_) = xen_test_utils.get_time(session, time_command,
                                                   time_filter_re, time_format)
            # Run current iteration
            logging.info("Rebooting: iteration %d of %d..." %
                         (i + 1, reboot_iterations))
            session = xen_test_utils.reboot(vm, session, timeout=login_timeout)
            # Get time after current iteration
            (ht1_, gt1_) = xen_test_utils.get_time(session, time_command,
                                                   time_filter_re, time_format)
        ...
        # Get final time
        (ht1, gt1) = xen_test_utils.get_time(session, time_command,
                                             time_filter_re, time_format)
    ...
    guest_delta = gt1 - gt0
    drift = abs(host_delta - guest_delta)
    drift_per = 100.0 * (host_delta - guest_delta) / host_delta
    ...
    test.write_perf_keyval({'timedrift':drift_per})
