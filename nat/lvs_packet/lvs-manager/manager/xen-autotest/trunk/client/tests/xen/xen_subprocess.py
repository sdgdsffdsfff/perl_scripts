...
def run_bg(command, termination_func=None, output_func=None, output_prefix="",
    ...
    @return: A xen_tail object.
    """
    process = xen_tail(command=command,
...
class xen_spawn:
    ...
    The pipes are requested and accessed by classes derived from xen_spawn.
    ...
    def __init__(self, command=None, id=None, echo=False, linesep="\n"):
        ...
        """
        self.id = id or xen_utils.generate_random_string(8)
    ...
    def close(self, sig=signal.SIGKILL):
        ...
        if self.is_alive():
            xen_utils.kill_process_tree(self.get_pid(), sig)
        ...
        for filename in (_get_filenames("/tmp/xen_spawn", self.id) +
...
class xen_tail(xen_spawn):
    ...
    def __init__(self, command=None, id=None, echo=False, linesep="\n",
        ...
        self._add_close_hook(xen_tail._join_thread)

        # Init the superclass
        xen_spawn.__init__(self, command, id, echo, linesep)
    ...
    def __getinitargs__(self):
        return xen_spawn.__getinitargs__(self) + (self.termination_func,
...
class xen_expect(xen_tail):
    ...
    def __init__(self, command=None, id=None, echo=False, linesep="\n",
        ...
        # Init the superclass
        xen_tail.__init__(self, command, id, echo, linesep,
                          termination_func, termination_params,
                          output_func, output_params, output_prefix)


    def __getinitargs__(self):
        return xen_tail.__getinitargs__(self)
    ...
    def read_until_output_matches(self, patterns, filter=lambda x: x,
        ...
        if match is None and (self.is_alive() or self.get_status() != 0):
            logging.debug("Timeout elapsed or process terminated. Output:" +
                          xen_utils.format_str_for_message(data.strip()))
...
class xen_shell_session(xen_expect):
    """
    ...
    It provides all services of xen_expect and xen_tail.  In addition, it
    ...
    def __init__(self, command=None, id=None, echo=False, linesep="\n",
        ...
        """
        # Init the superclass
        xen_expect.__init__(self, command, id, echo, linesep,
                            termination_func, termination_params,
                            output_func, output_params, output_prefix)
    ...
    def __getinitargs__(self):
        return xen_expect.__getinitargs__(self) + (self.prompt,
    ...
    def get_command_status_output(self, command, timeout=30.0,
        ...
        if status != 0:
            logging.debug("Command failed; status: %d, output:%s", status,
                          xen_utils.format_str_for_message(output.strip()))
